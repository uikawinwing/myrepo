...
async function fetchProjectEntries(projectOrId, options = {}) {
  try {
    ...
    const request = apiFetch('/api/projects/' + projectId).then(detail => {
      const data = {
        project: detail.project || projectOrId,
        entries: Array.isArray(detail.worldbookEntriesPreview) ? detail.worldbookEntriesPreview : [],
        regexEntries: Array.isArray(detail.regexEntriesPreview) ? detail.regexEntriesPreview : [],
      };
